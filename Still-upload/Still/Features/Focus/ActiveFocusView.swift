            ...
            ScrollView {
                VStack(spacing: StillTheme.Spacing.l) {
                    PixelSceneView(scene: scene, mode: session.renderMode, intensity: appState.animationIntensity)
                        ...
                        // keeps the frame readable instead of melting into the page.
                        .overlay(
                            RoundedRectangle(cornerRadius: StillTheme.Radius.scene, style: .continuous)
                                .strokeBorder(onScene ? StillTheme.Palette.sceneText.opacity(0.14) : StillTheme.border,
                                              lineWidth: StillTheme.Stroke.hairline)
                        )
