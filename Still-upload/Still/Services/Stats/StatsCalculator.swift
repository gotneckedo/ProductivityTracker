...
struct StatsCalculator {
    ...
    func stats(sessions: [FocusSession], usages: [ActivityUsage], now: Date) -> FocusStats {
        ...
        let ranges = Dictionary(uniqueKeysWithValues: FocusStatsRange.allCases.map { range in
            (range, rangeData(range, completed: completed, usualDailyFocus: usual, now: now))
        })
    ...
    func rangeData(
        ...
        let completed = sessions.filter { $0.state == .completed && $0.endedAt != nil }
        return rangeData(range, completed: completed, usualDailyFocus: usualDailyFocus(completed: completed, now: now), now: now)
    }
    ...
    private func rangeData(
        ...
        let points = points(for: range, interval: interval, completed: sessionsInRange)
    ...
    private func points(
        ...
        switch range {
        case .day:
            return [point(start: interval.start, end: interval.end, completed: completed)]
        case .week, .month:
            var result: [FocusRangePoint] = []
            var cursor = interval.start
            while cursor < interval.end {
                let end = calendar.date(byAdding: .day, value: 1, to: cursor)!
                result.append(point(start: cursor, end: end, completed: completed))
                cursor = end
            }
            return result
        case .year:
    ...
    private func point(start: Date, end: Date, completed: [FocusSession]) -> FocusRangePoint {
